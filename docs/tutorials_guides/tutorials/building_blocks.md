...
该类[`Scene`]()是Manim库的核心组件。每个对象都必须[`added`]()指向一个要显示的场景，或者[`removed`]()从该场景停止显示。每个动画都必须 [`played`]()由一个场景组成，并且每个不发生动画的时间间隔都由对 的调用确定[`wait()`]()。视频的所有代码必须包含在[`construct()`]()派生自 的类的方法中[`Scene`]()。[`Scene`]()最后，如果要同时渲染多个场景，单个文件可能包含多个子类。
